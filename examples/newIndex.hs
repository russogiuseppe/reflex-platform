...
main =
  let gGetById f d i = fmap f <$> D.getElementById d i
      ...
      setFunction idNum doc = do 
        (Just (idn :: String)) <- GMI.fromJSVal idNum
        request <- O.create
        delete <- GMI.toJSVal "DELETE"
        O.setProp (T.toJSString "method") delete request;
        o <- fetch' (endpoint ++ "/" ++ idn) request
        cb <- (asyncCallback1 $ \_ -> do{
          qut <- uGetById doc idn;
